...
export const handlers = [
  http.post("http://localhost:3000/v1/login/local", async (req) => {
  ...
  http.post(
    "http://localhost:3000/v1/signatures/upload",
    async (req) => {
      const body = await req.request.json();
  ...
  http.post("http://localhost:3000/v1/signatures/12A/request-approval", async (req) => {
    return HttpResponse.json({
      score: Math.random(),
    });
  }),
  ...
  http.get("http://localhost:3000/v1/accounts/me", () => {
    return HttpResponse.json(
      JSON.parse(localStorage.getItem("loginAccount") || "{}")
    );
  }),
  http.get(
    "http://localhost:3000/v1/signatures?size=25&page=0",
    () => {
